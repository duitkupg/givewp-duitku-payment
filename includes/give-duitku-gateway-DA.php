...
class DuitkuGatewayDA extends PaymentGateway {
	/**
	...
	public function getName(): string {
		return __( 'Duitku DANA', 'give-duitku' );
	}
	...
	public function getLegacyFormFieldMarkup( $formId, $args ) {
		return "<div class=''>
            <p>You will be redirected to Duitku DANA Payment Gateway</p>
        </div>";
	}
	...
	public function enqueueScript( int $formId ) {
		...
		wp_enqueue_script(
			$handle,
			plugin_dir_url( __FILE__ ) . 'js/duitku-DA-gateway.js',
			[ 'react', 'wp-element' ],
			'1.0.0',
			true );
	...
	public function createPayment( Donation $donation, $gatewayData ): RedirectOffsite {
		...
		if ($response_code == '200') {
			...
			if ($resp->paymentUrl == '') {
				throw new PaymentGatewayException(esc_html__($resp->statusMessage, 'give-duitku' ));
			} else {
