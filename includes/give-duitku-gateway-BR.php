...
class DuitkuGatewayBR extends PaymentGateway {
	/**
	...
	public function getName(): string {
		return __( 'Duitku BRIVA', 'give-duitku' );
	}
	...
	public function getLegacyFormFieldMarkup( $formId, $args ) {
		return "<div class=''>
            <p>You will be redirected to Duitku BRI Virtual Account Payment Gateway</p>
        </div>";
	}
	...
	public function enqueueScript( int $formId ) {
		...
		wp_enqueue_script(
			$handle,
			plugin_dir_url( __FILE__ ) . 'js/duitku-BR-gateway.js',
			[ 'react', 'wp-element' ],
			'1.0.0',
			true );
	}
