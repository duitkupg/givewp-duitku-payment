...
class DuitkuGatewayNQ extends PaymentGateway {
	/**
	...
	public function getName(): string {
		return __( 'Duitku Nobu', 'give-duitku' );
	}
	...
	public function getLegacyFormFieldMarkup( $formId, $args ) {
		return "<div class=''>
            <p>You will be redirected to Duitku Nobu Payment Gateway</p>
        </div>";
	}
	...
	public function enqueueScript( int $formId ) {
		...
		wp_enqueue_script(
			$handle,
			plugin_dir_url( __FILE__ ) . 'js/duitku-NQ-gateway.js',
			[ 'react', 'wp-element' ],
			'1.0.0',
			true );
	}
